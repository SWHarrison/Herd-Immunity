class Logger(object):
    ...
    def log_interaction(self, person1, person2, did_not_infect):
        ...
        if(did_not_infect):
            if(person2.is_vaccinated):
                file.write("Person " + str(person1._id) + " did not infect person " + str(person2._id) + " due to vaccination." + "\n")
            elif(person2.infection != None):
                file.write("Person " + str(person1._id) + " did not infect person " + str(person2._id) + " due to prior infection." + "\n")
            else:
                file.write("Person " + str(person1._id) + " did not infect person " + str(person2._id) + " due to failure of infection to reproduce." + "\n")
        else:
    ...
    def log_infection_survival(self, person, did_die_from_infection):
        ...
        if(did_die_from_infection):
            file.write("Person " + str(person._id) + " died from infection." + "\n")
        else:
            file.write("Person " + str(person._id) + " survived the infection." + "\n")

        file.close()

    def log_time_step(self, time_step_number, total_dead, new_infected):
        ...
        file.write("Time step: " + str(time_step_number) + ", Total dead: " + str(total_dead) + ", Total infected (not dead): " + str(new_infected) + "\n")

        file.close()

    def log_infection(self, person):
        file = open(self.file_name,"a")

        file.write("Person " + str(person._id) + " was infected." + "\n")
